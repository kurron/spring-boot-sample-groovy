...
[[overview]]
= Overview
The Hash ID (HID) Generator service is a simple microservice that calculates a message digest (cryptographic hash) of a particular card.  The algorithm
...
HID Generator tries to adhere as closely as possible to standard HTTP and REST conventions in its
...
| `X-Correlation-Id`
| HID Generator is part of a network of cooperating services and publishes its log messages to a centralized server.  To facilitate the tracing of events
...
HID Generator uses a simple hypermedia control based on http://stateless.co/hal_specification.html[HAL] and includes links to other resources that the client
...
HID Generator does not currently require any form of authentication, expecting that the service will not be
exposed directly to the public internet.  HID Generator does not currently rate limit requests, expecting that
...
The `Accept` and `Content-Type` headers contain the media types of entities being sent to and returned from the service.  HID Generator makes
...
| `Accept`
| This header is used to indicate the format of the response.  In the context of HID Generator, it is always the MIME type of the hypermedia control.
| `Content-Type`
